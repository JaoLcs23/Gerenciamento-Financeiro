...
public class RecurringTransactionController extends BaseController {
    ...
    @FXML private TableColumn<TransacaoRecorrente, Conta> colAccount;
    ...
    private void handleAddOrUpdateRecurringTransaction(ActionEvent event) {
        ...
        } catch (NumberFormatException e) { showFieldError(valueField, valueErrorLabel, "Valor inválido."); isValid = false; }
        try {
            diaDoMes = Integer.parseInt(diaDoMesField.getText());
            if (diaDoMes < 1 || diaDoMes > 31) { showFieldError(diaDoMesField, diaDoMesErrorLabel, "Dia deve ser entre 1 e 31."); isValid = false; }
        } catch (NumberFormatException e) { showFieldError(diaDoMesField, diaDoMesErrorLabel, "Dia inválido."); isValid = false; }
        if (type == null) { showFieldError(typeComboBox, typeErrorLabel, "Tipo é obrigatório."); isValid = false; }
        if (category == null) { showFieldError(categoryComboBox, categoryErrorLabel, "Categoria é obrigatória."); isValid = false; }
